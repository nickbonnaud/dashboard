...
void main() {
  group("Hours Screen Tests", () {
    ...
    Future<void> _setupSelectionForm({required WidgetTester tester}) async {
      ...
      await tester.tap(find.byKey(Key("openingTimeFieldKey")));
      await tester.pump();
      ...
      await tester.tap(find.byKey(Key("closingTimeFieldKey")));
      await tester.pump();
    ...
    Future<void> _setUpConfirmationForm({required WidgetTester tester}) async {
      ...
      await tester.tap(find.byKey(Key("toggleAllButtonKey")));
      await tester.pump();
      await tester.drag(find.text("Drag or click to select hours."), Offset(0.0, -500));
      await tester.pump();
    ...
    testWidgets("MaxHoursForm contains earliest opening TextFormField", (tester) async {
      await mockNetworkImagesFor(() => screenBuilder.createScreen(tester: tester));
      expect(find.byKey(Key("openingTimeFieldKey")), findsOneWidget);
    });

    testWidgets("MaxHoursForm contains latest closing TextFormField", (tester) async {
      await mockNetworkImagesFor(() => screenBuilder.createScreen(tester: tester));
      expect(find.byKey(Key("closingTimeFieldKey")), findsOneWidget);
    });

    testWidgets("Tapping EarliestOpeningTextField displays DatePicker", (tester) async {
      await mockNetworkImagesFor(() => screenBuilder.createScreen(tester: tester));
      expect(find.byKey(Key("timePickerKey")), findsNothing);
      await tester.tap(find.byKey(Key("openingTimeFieldKey")));
      await tester.pump();
      expect(find.byKey(Key("timePickerKey")), findsOneWidget);
    });
    ...
    testWidgets("Setting DatePicker for EarliestOpeningTextField changes textField", (tester) async {
      await mockNetworkImagesFor(() => screenBuilder.createScreen(tester: tester));
      await tester.pump();
      expect(tester.widget<TextFormField>(find.byKey(Key("openingTimeFieldKey"))).controller?.text.isEmpty, true);
      await tester.tap(find.byKey(Key("openingTimeFieldKey")));
      await tester.pump();
      await tester.tap(find.text("Set"));
      await tester.pump();
      expect(tester.widget<TextFormField>(find.byKey(Key("openingTimeFieldKey"))).controller?.text.isEmpty, false);
    });

    testWidgets("Setting DatePicker for LatestClosingTextField changes textField", (tester) async {
      await mockNetworkImagesFor(() => screenBuilder.createScreen(tester: tester));
      await tester.pump();
      expect(tester.widget<TextFormField>(find.byKey(Key("closingTimeFieldKey"))).controller?.text.isEmpty, true);
      await tester.tap(find.byKey(Key("closingTimeFieldKey")));
      await tester.pump();
      await tester.tap(find.text("Set"));
      await tester.pump();
      expect(tester.widget<TextFormField>(find.byKey(Key("closingTimeFieldKey"))).controller?.text.isEmpty, false);
    });
    ...
    testWidgets("Hours Selection Form contains Hours Selection Grid", (tester) async {
      await _setupSelectionForm(tester: tester);
      expect(find.byKey(Key("hoursSelectionGrid")), findsOneWidget);
    });

    testWidgets("Hours Selection Form creates toggleAllButton", (tester) async {
      await _setupSelectionForm(tester: tester);
      expect(find.byKey(Key("toggleAllButtonKey")), findsOneWidget);
    });
    ...
    testWidgets("Tapping empty grid tile changes selects time", (tester) async {
      await _setupSelectionForm(tester: tester);
      expect(tester.widget<Container>(find.byKey(Key("empty-1-1"))).color, Colors.white);
      await tester.tap(find.byKey(Key("empty-1-1")));
      await tester.pump();
      expect(tester.widget<Container>(find.byKey(Key("filled-1-1"))).color, Color(0xFF016fb9));
    });
    ...
    testWidgets("Hours Selection Form creates Reset Max Hours Button", (tester) async {
      await _setupSelectionForm(tester: tester);
      expect(find.byKey(Key("resetMaxHoursButtonKey")), findsOneWidget);
    });
    ...
    testWidgets("Tapping Reset Max Hours Button Key goes to MaxHoursForm", (tester) async {
      ...
      await tester.drag(find.text("Drag or click to select hours."), Offset(0.0, -500));
      await tester.pump();
      await tester.tap(find.byKey(Key("resetMaxHoursButtonKey")));
      await tester.pump();
      expect(find.byType(MaxHoursForm), findsOneWidget); 
    });
    ...
    testWidgets("HoursConfirmationForm creates hours list", (tester) async {
      ...
      expect(find.byKey(Key("hoursList")), findsOneWidget);
    });
    ...
    testWidgets("HoursConfirmationForm creates goBackButton", (tester) async {
      await _setUpConfirmationForm(tester: tester);
      expect(find.byKey(Key("goBackButtonKey")), findsOneWidget);
    });

    testWidgets("Tapping GoBackButton hides HoursConfirmationForm and shows HoursSelectionForm", (tester) async {
      await _setUpConfirmationForm(tester: tester);
      expect(find.byType(HoursConfirmationForm), findsOneWidget);
      expect(find.byKey(Key("hoursSelectionFormKey")), findsNothing);
      await tester.drag(find.byKey(Key("scrollKey")), Offset(0.0, -100));
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(Key("goBackButtonKey")));
      await tester.pump();
      expect(find.byType(HoursConfirmationForm), findsNothing);
      expect(find.byKey(Key("hoursSelectionFormKey")), findsOneWidget);
    });
