...
void main() {
  group("Sales Screen Tests", () {
    ...
    testWidgets("NetSales widget displays correct net sales amount", (tester) async {
      ...
      when(() => transactionRepository.fetchNetSalesToday())
        .thenAnswer((_) async => Future.delayed(Duration(milliseconds: 500), () => netSales));
      
      await screenBuilder.createScreen(tester: tester);
      expect(find.text(appCurrency.Currency.create(cents: netSales)), findsOneWidget);
    });
    ...
    testWidgets("TotalSales widget displays correct total sales amount", (tester) async {
      ...
      when(() => transactionRepository.fetchTotalSalesToday())
        .thenAnswer((_) async => Future.delayed(Duration(milliseconds: 500), () => totalSales));
      
      await screenBuilder.createScreen(tester: tester);
      expect(find.text(appCurrency.Currency.create(cents: totalSales)), findsOneWidget);
    });
    ...
    testWidgets("TotalTaxes widget displays correct total taxes amount", (tester) async {
      ...
      when(() => transactionRepository.fetchTotalTaxesToday())
        .thenAnswer((_) async => Future.delayed(Duration(milliseconds: 500), () => totalTaxes));
      
      await screenBuilder.createScreen(tester: tester);
      expect(find.text(appCurrency.Currency.create(cents: totalTaxes)), findsOneWidget);
    });
