...
class DateRangePicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TransactionsListBloc, TransactionsListState>(
      builder: (context, state) {
        if (state.currentFilter == FilterType.transactionId) return Container();
        return _button(context: context, state: state);
      }
    );
  }

  Widget _button({required BuildContext context, required TransactionsListState state}) {
    return IconButton(
      icon: Icon(
        Icons.date_range,
  ...
  void _datePickerPressed({required BuildContext context, required TransactionsListState state}) async {
